...
void CsvGenerator::processDebugInfo(const Module &Mod)
{
          ...
          case dwarf::Tag::DW_TAG_typedef:
              {
                  ...
                  if (const DICompositeType *compType = dyn_cast<DICompositeType>(baseType))
                      processCompTypeDebug(*compType, dbgType.getName());
              }
              break;
          case dwarf::Tag::DW_TAG_structure_type:
          case dwarf::Tag::DW_TAG_class_type:
              processCompTypeDebug(cast<DICompositeType>(dbgType));
              break;
          default:
              break;
        }
...
void CsvGenerator::processCompTypeDebug(const DICompositeType &type, const string altName)
{
    // Construct refmode
    refmode_t refmode;
